...
################################
# Modified by Jacob Cox
# Date: 21 Dec 2016
"""
Some switches do not send enough data to the controller for the original
dhcp.py file to work. This modification allows network operators to
capture all of the packet data below minus options, file, and part of
sname (only captures 40 bytes of sname)
"""
################################
...
class dhcp(packet_base.PacketBase):
    ...
    * RFC 2131 DHCP packet format
    ...
    #(mod) Created second option for unpacking string
    ...
    def _parser(cls, buf):
        ...
        #(mod) provided option for smaller packet sizes
        if len(buf) < 100:
        ...
        #(mod) provided option for smaller packets, set bootfile to b''
        if min_len > 235:
